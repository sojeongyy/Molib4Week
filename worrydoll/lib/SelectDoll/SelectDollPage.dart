    ...
    return Container(
      decoration: BoxDecoration(
        image: DecorationImage(
          image: AssetImage('assets/images/wood_background.jpg'), // 배경 이미지
          fit: BoxFit.cover,
        ),
      ),
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: _selectedDollName != null
                ? () {
              // 결정 버튼 동작: 인형 선택 후에만 팝업 표시
              showDialog(
                context: context,
                builder: (context) => AlertDialog(
                  title: Text('결정'),
                  content: Text('${_selectedDollName} 인형을 선택했습니다!'),
                  actions: [
                    TextButton(
...
// -----------------------------------------------------------------
// DollImage: 흔들림 애니메이션 + 선택 시 크기 증가 추가
// -----------------------------------------------------------------
class DollImage extends StatefulWidget {
  final String imagePath;
  final VoidCallback onTap;

  // 기존 코드에 없던 부분: 인형이 선택된 상태인지 확인
